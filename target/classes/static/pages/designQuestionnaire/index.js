...
const handleEditFinish = () => {
  let params = $util.getItem("questObj");
  if(!params){
    alert("请检查！【无可用参数】");
    return ;
  }
  params.questionTitle=$('.questionnaire-title span' ).text().trim();
  params.context=$('.questionnaire-description span' ).text().trim();

  var elements = $('.bottom').filter(function() {
    return $(this).css('display') === 'block';
  });

  if(elements.length>0){
    return alert("请先完成未完成的编辑");
  }
  debugger
  var obj={
    item1:[],
  ...
  $('.question[data-type="1"]').each(function() {
    var questionTitle = $(this).find('#questionTitle').text().trim();
    var el={};
    el.title=questionTitle;
    el.child=[];
    $(this).find('.radio-inline').each(function () {
      el.child.push( $(this).text().trim());
    })
    obj.item1.push(el);
  });
  //多选
  $('.question[data-type="2"]').each(function() {
    var questionTitle = $(this).find('#questionTitle').text().trim();
    var el={};
    el.title=questionTitle;
    el.child=[];
    $(this).find('.checkbox-inline').each(function () {
      el.child.push( $(this).text().trim());
    })
    obj.item2.push(el);
  });
  //填空
  $('.question[data-type="3"]').each(function() {
    var questionTitle = $(this).find('#questionTitle').text().trim();
    var el={};
    el.title=questionTitle;
    obj.item3.push(el);
  ...
  //矩阵
  $('.question[data-type="4"]').each(function() {
    var questionTitle = $(this).find('#questionTitle').text().trim();
    var el={};
    el.title=questionTitle;
    el.table=$(this).find('.table').html();
    obj.item4.push(el);
  });
  //量表
  $('.question[data-type="5"]').each(function() {
    var questionTitle = $(this).find('#questionTitle').text().trim();
    var el={};
    el.title=questionTitle;
    el.table= $(this).find('.bottom2').html()
    // $(this).find('.bottom2').each(function () {
    //   var text=$(this).text().trim();
    //   var cleanedText = text.match(/\b\w+\b/g);
    //   el.child.push(cleanedText)
    // })
    obj.item5.push(el);
  });
  params.question=JSON.stringify(obj);
  $.ajax({
    url: API_BASE_URL + '/admin/modifyQuestionnaireOrInsert',
    type: "POST",
    data: JSON.stringify(params),
    dataType: "json",
    contentType: "application/json",
    success(res) {
       $util.setItem("questObj",null);
       alert("问卷创建成功")
    }
  })
}
